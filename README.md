...
3. 控制端从自动发现的设备列表选择主机，或在“添加设备”中输入识别码，输入对方访问密码后直接进入桌面。
...
访问密码为随机生成的 128 位秘密（22 个字符，可复制粘贴），不会作为明文查询参数发送。首次授权复用 GameStream 双向挑战和证书交换；设备识别码不是授权凭据。新增授权仅接受本机或私有网络地址，并限制一分钟内的尝试次数。停止共享后点击“重置访问权限”，会更换密码并撤销全部已保存的客户端授权；仅停止共享不会清除授权。
