...
<img src="app/res/desk.svg" alt="Desk icon" width="80" height="80">
...
收起时为 48 × 20 的小箭头，展开为 584 × 44 的单行控制条（逻辑尺寸，随屏幕缩放）。macOS 使用更轻透的原生毛玻璃，文字与图标保持不透明；Windows／X11 展开时不透明度为 88%，收起时为 80%。切换焦点或重建串流窗口后会重新同步工具栏的位置和层级。
...
选择“鼠标：游戏”后，工具栏仍可操作，点击远程画面或收起工具栏才恢复鼠标捕获。游戏模式的顶部小条会显示返回快捷键；鼠标被捕获时按 `Ctrl+Alt+Shift+T` 会释放鼠标并展开工具栏。
...
- 串流顶部悬浮栏点击“传文件”，或在设备的“管理 → 文件传输”打开双栏窗口。左侧浏览本机，右侧浏览远端共享目录。
- 勾选多个文件或文件夹后点击上传／下载，也可在两栏之间拖拽。操作为复制，保留源文件；支持递归文件夹、空文件、中文文件名和空目录。
- 同名文件可覆盖、跳过或保留两份。任务队列显示进度、速度和错误，保留最近 100 条已结束记录，支持取消、重新尝试；重试从头开始，暂不支持断点续传。
- 默认限速 4 MB/s，可选择 1／4／10 MB/s 或不限速，减少与桌面串流争用带宽。关闭正在传输的窗口后任务继续，点击入口可重新打开；结束 Desk 主机服务会中断相关传输。
- 被控端默认只共享“下载/Desk”，在“本机”页停止服务后可更改路径。已授权设备可读写该目录，其他路径、符号链接及 Windows 重解析点不开放。两端均需升级，原版 Sunshine／旧版 Desk 会提示不支持。
- 复用主机的 HTTPS 端口和已配对客户端证书，客户端固定校验主机证书；仅允许本机／私有网络地址。分块传输限制内存，上传校验 SHA-256，完整文件才提交至目标位置。取消或失败会丢弃临时文件，不直接覆盖原文件。

文件窗口以独立进程运行，避免 SDL 串流循环阻塞 Qt 界面；打开时释放远程输入，若当前全屏则先切换为窗口。6.3.0 支持目录浏览与复制；系统文件剪贴板与原生拖拽在下述 6.4.0 扩展中提供。暂不提供删除远端文件或剪切移动。

原生测试：先编译 Sunshine 以准备 JSON 依赖，再运行 `python3 scripts/test-file-transfer.py`。CI 在 Windows、macOS、Ubuntu 和 Debian 执行双向传输、目录、冲突、取消、QML 窗口与拖拽测试；安装包另执行真实 Sunshine 的未授权拒绝及文件协议检查。

### 系统文件剪贴板与拖拽（6.4.0）

- 保留双栏传输窗口。可从 Finder／资源管理器／X11 文件管理器拖入右侧目录上传，也可在右侧目录粘贴本机已复制的文件。
- 从 Desk 文件列表拖出窗口，可投放到本机桌面或文件管理器目录。远端文件在目标请求内容时分块下载，保存到实际投放位置；不包含从串流画面内部直接拖出图标。
- 串流期间同步文件剪贴板：一端复制文件或目录，另一端在系统文件管理器粘贴。断开会话后撤销文件访问；每个登录用户同一时间只启用一个 Desk 剪贴板会话。
- 剪贴板只授权用户明确选中的文件，不改变原有共享目录边界。符号链接和 Windows 重解析点不开放；重新复制或来源变化会使旧清单失效。
- macOS 使用原生文件承诺实现拖出；文件粘贴通过带随机标记的临时占位项识别目标后写入。粘贴目录合并遇到已有子文件时会停止，不静默覆盖。
- Windows 使用 OLE 文件描述和按需读取流；Linux 仅支持 X11，使用 FUSE3 和文件 URI。DEB 会安装 FUSE3，当前用户还需能使用 `/dev/fuse`；Wayland 不在本轮支持范围内。
- 原生传输的内容读取进度显示在文件窗口队列中，可以取消；重试请重新复制或拖拽。系统文件管理器负责最终落盘提示，队列中的“内容已传输”表示内容已经交付给接收方。
- 两端均需新版 Desk 并重新启动主机服务。自动化覆盖授权、清单失效、分块读取、取消、原生提供器和界面回归；Finder／Explorer／X11 文件管理器跨机器交互仍需实机验收。

6.4.1 修复文件接口的 HTTPS 连接收尾阻塞；两端升级后请重启主机服务。文件窗口默认 800×580，主窗口与文件窗口使用同一图标。

6.4.2 将内置主机服务绑定到 Desk 的进程生命周期：正常退出或主程序崩溃后自动收尾，卡住时最多等待 5 秒后退出，避免残留进程占用端口。此机制需使用新版内置 Sunshine；旧版已经遗留的进程仍需结束一次。自动发现不再显示本机，之前保存的本机识别码记录也从设备列表中隐藏。

### 署名与开源许可

Desk 由 coderleex 维护，基于 Moonlight Qt，并内置 Sunshine。`Desk modifications © 2026 coderleex` 仅说明 Desk 修改部分的版权；上游代码及第三方组件的版权仍归各自权利人所有，原有版权与许可声明继续保留。

点击侧栏底部的“版本号 · 关于”可查看上游项目、Desk 源码和离线 GPLv3 许可证全文。Desk 按仓库的 GPLv3 许可发布，不提供担保；各第三方组件另遵循其随附许可证。
